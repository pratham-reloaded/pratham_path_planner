...
std::tuple<bool, std::vector<Grid>> GeneticAlgorithm::Plan(const Grid &start,
                                                           const Grid &goal) {
  int generation = 0;
  start_ = start;
  goal_ = goal;
  ...
  std::vector<Grid> initial_path = GenerateSimplePath();
  ...
  while (generation <= generations_) {
    ...
    std::transform(
        ...
        [&](const std::vector<Grid> &path) { return CalculateFitness(path); });

    f_val = *std::min_element(f_vals.begin(), f_vals.end());
    std::vector<std::vector<Grid>> new_paths_;
    ...
    for (int i = 0; i < paths_.size(); ++i) {
      ...
      if (f_vals[i] <= f_val * c_) {
        if (shorten_chromosome_) {
          ...
          new_paths_.emplace_back(std::vector<Grid>(
...
std::vector<Grid> GeneticAlgorithm::ReturnLastPath() const {
  if (truepaths_.empty()) {
    return {};
  }
...
void GeneticAlgorithm::CheckIfGridsInPathAreAcceptable(
    const std::vector<Grid> &path) const {
...
void GeneticAlgorithm::PrintChromosome(const std::vector<Grid> &path) const {
  std::cout << "Chromosome: ";
...
std::vector<Grid> GeneticAlgorithm::GenerateSimplePath() const {
  int d_x = goal_.x_ - start_.x_;
  int d_y = goal_.y_ - start_.y_;
  Grid dx(d_x >= 0 ? 1 : -1, 0);
  Grid dy(0, d_y >= 0 ? 1 : -1);
  std::vector<Grid> path(path_length_);
  auto it = path.begin();
...
int GeneticAlgorithm::CalculateFitness(const std::vector<Grid> &path) const {
  int cost = 0;
  Grid i = start_;
...
std::vector<Grid> GeneticAlgorithm::GenerateRandomPath() const {
...
std::vector<Grid> GeneticAlgorithm::Mutate() const {
  std::random_device rd;
  std::mt19937 eng(rd());
...
std::vector<Grid> GeneticAlgorithm::Crossover() const {
...
bool GeneticAlgorithm::CheckPath(const std::vector<Grid> &path) const {
  Grid current = start_;
  if (CompareCoordinates(current, goal_)) {
    return true;
  }
