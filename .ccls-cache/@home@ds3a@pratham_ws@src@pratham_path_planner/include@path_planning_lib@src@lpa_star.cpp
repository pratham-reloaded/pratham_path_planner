...
bool LPAStar::IsObstacle(const Grid& n) const { return grid_[n.x_][n.y_] == 1; }

double LPAStar::H(const Grid& n1, const Grid& n2) {
  return std::sqrt(std::pow(n1.x_ - n2.x_, 2) + std::pow(n1.y_ - n2.y_, 2));
}

std::vector<Grid> LPAStar::GetNeighbours(const Grid& u) const {
...
std::vector<Grid> LPAStar::GetPred(const Grid& u) const {
  return GetNeighbours(u);
}

std::vector<Grid> LPAStar::GetSucc(const Grid& u) const {
  return GetNeighbours(u);
}

double LPAStar::C(const Grid& s1, const Grid& s2) const {
...
Key LPAStar::CalculateKey(const Grid& s) const {
...
void LPAStar::Initialize() {
  ...
  U_.insert(GridKeyPair{start_, CalculateKey(start_)});
}

void LPAStar::UpdateVertex(const Grid& u) {
...
std::vector<Grid> LPAStar::DetectChanges() {
...
void LPAStar::ClearPathDisplay(const std::vector<Grid>& path) {
...
void LPAStar::UpdatePathDisplay(const std::vector<Grid>& path) {
  for (const auto& node : path) {
    grid_[node.x_][node.y_] = 3;
  }
}

std::vector<Grid> LPAStar::GetNewPath() {
  auto current = goal_;
...
std::tuple<bool, std::vector<Grid>> LPAStar::Plan(const Grid& start,
                                                  const Grid& goal) {
