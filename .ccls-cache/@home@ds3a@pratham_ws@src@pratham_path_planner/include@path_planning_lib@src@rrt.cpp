...
// constants
constexpr double half_grid_unit = 12.9;
constexpr double precision_limit = 0.0001;

std::tuple<bool, Grid> RRT::FindNearestPoint(Grid& new_node) {
  bool found = false;
  Grid nearest_node;
...
bool RRT::IsAnyObstacleInPath(const Grid& n_1, const Grid& n_2) const {
  if (n_2.y_ - n_1.y_ == 0) {
...
Grid RRT::GenerateRandomGrid() const {
...
std::tuple<bool, std::vector<Grid>> RRT::Plan(const Grid& start,
                                              const Grid& goal) {
  start_ = start;
  goal_ = goal;
...
bool RRT::CheckGoalVisible(const Grid& new_node) {
...
std::vector<Grid> RRT::CreatePath() {
