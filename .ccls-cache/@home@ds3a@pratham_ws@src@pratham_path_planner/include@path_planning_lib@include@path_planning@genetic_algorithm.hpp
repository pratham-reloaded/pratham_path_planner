...
class GeneticAlgorithm : public Planner {
 public:
   ...
   */
  std::vector<Grid> ReturnLastPath() const;
   ...
   * @return void
   */
  void PrintChromosome(const std::vector<Grid>& path) const;
   ...
   * @return void
   */
  void PrintPathOfChromosome(const std::vector<Grid>& path) const;
   ...
   * @return chromosome
   */
  std::vector<Grid> GenerateSimplePath() const;

  /**
   * @brief Creates a random chromosome
   * @return chromosome
   */
  std::vector<Grid> GenerateRandomPath() const;
  /**
   * @brief Calculates fitness value of a path
   * @param path Each chromosome represents a path by a sequence of moves.
   * @return value of fitness
   */
  int CalculateFitness(const std::vector<Grid>& path) const;
   ...
   */
  std::vector<Grid> Crossover() const;
   ...
   */
  bool CheckPath(const std::vector<Grid>& path) const;
   ...
   * @return void
   */
  void CheckIfGridsInPathAreAcceptable(const std::vector<Grid>& path) const;
 ...
 private:
  ...
  int popsize_ = 30;
  double c_ = 1.05;
  std::vector<std::vector<Grid>> paths_;
  std::vector<std::vector<Grid>> truepaths_;
  bool found_ = false;
