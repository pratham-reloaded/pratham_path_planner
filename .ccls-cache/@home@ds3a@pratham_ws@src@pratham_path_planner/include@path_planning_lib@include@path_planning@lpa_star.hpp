...
class LPAStar : public Planner {
 ...
 private:
   ...
   * @param s Grid for whicht eh key values are to be calculated
   ...
   * @param u Grid for which the predecessors are to be found
   ...
   * @param u Grid for which the successors are to be found
   * @return predecessors of node u
   * @details implemented for a grid with undirected edges,
   ...
   * @param u Grid for which neighbours are to be found
   * @return neighbours
  ...
  void ClearPathDisplay(const std::vector<Grid>& path);
  std::vector<Grid> GetNewPath();
  void UpdatePathDisplay(const std::vector<Grid>& path);
